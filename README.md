...
- **录入方式（三选一）**：
  - **粘贴剪贴板**：主页输入框旁「粘贴」按钮，一键读入剪贴板里的分享链接
  - **扫码**：「扫码」按钮调用相机扫描分享链接二维码，自动填入
  - **Deep Link 深链**：在浏览器打开夸克分享链接并点「打开 APP / 去 APP 查看」，自动唤起本应用并把该分享链接填入输入框（兼容 `qklink://`、`qkcloudlink://`、`uclink://`、`quark://` 多种 scheme，自动解析多层 URL 编码中的分享码 `pwd_id`）
- **批量转存**：主页粘贴分享链接，每行一个，支持批量
...
- **主页**：顶部标题栏（含设置齿轮入口）+「粘贴分享链接」说明 + 分享链接输入框（旁有「粘贴」「扫码」按钮）+「开始转存」按钮 + 转存进度卡（进度条、百分比、实时日志）
